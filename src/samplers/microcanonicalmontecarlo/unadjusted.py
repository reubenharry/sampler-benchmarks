...
def unadjusted_mclmc_no_tuning(initial_state, integrator_type, step_size, L, sqrt_diag_cov, num_tuning_steps, return_ess_corr=False, return_samples=False):
    ...
    def s(model, num_steps, initial_position, key):
        ...
        return expectations, {
                "params": MCLMCAdaptationState(L=L, step_size=step_size, sqrt_diag_cov=sqrt_diag_cov),
...
def unadjusted_mclmc_tuning(initial_position, num_steps, rng_key, logdensity_fn, integrator_type, diagonal_preconditioning, frac_tune3=0.1, num_tuning_steps=500):
...
def unadjusted_mclmc(preconditioning=True, integrator_type='mclachlan', frac_tune3=0.1, return_ess_corr=False, num_tuning_steps = 500, return_samples=False):
    ...
    def s(model, num_steps, initial_position, key):
        ...
        ) = unadjusted_mclmc_tuning( initial_position, num_steps, tune_key, model.unnormalized_log_prob, integrator_type, preconditioning, frac_tune3, num_tuning_steps=num_tuning_steps)

...
def do_grid(func, param_grid, fixed_params=None, verbose= False, extra_word= ''): 
    """ Perform a grid search over specified parameters for a given function, while keeping 
    other parameters fixed to their default values, and allowing for parameters without 
    default values to be specified separately. Parameters: - func: The function to evaluate.
      It must return a dictionary. - param_grid: A dictionary where keys are parameter names and 
      values are lists of values to try. - fixed_params: A dictionary of parameters that don't 
      have default values, to be fixed across all evaluations. Returns: - A pandas DataFrame where 
      each row represents a parameter combination, excluding fixed parameters, with additional 
      columns corresponding to the keys of the dictionary returned by the function. """ 
    ...
    for values in combinations: 
        if verbose: 
            print(f'{counter} / {len(combinations)}') 
        ...
        row = { k: params[k] for k in sig.parameters.keys() } 
        row.update(result_dict) 
        # Add the function output dictionary 
        results.append(row) 

    print(f'{len(combinations)} / {len(combinations)}')

    # Convert results to a pandas 
